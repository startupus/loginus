...
export class AuthService {
  ...
  async login(
    ...
    | AuthResponseDto
    | { requires2FA: true; message: string; userId: string }
    | {
        requiresNFA: true;
        message: string;
        userId: string;
   ...
   * Поддерживает вход как по email, так и по номеру телефона.
   */
  async validateUser(login: string, password: string): Promise<User> {
    // Простое эвристическое определение: если строка содержит "@", считаем это email
    ...
    if (!user.passwordHash) {
      // Если пользователь не имеет пароля, но имеет primaryAuthMethod === 'EMAIL',
      // это означает, что он зарегистрировался через код и пароль не был установлен
      // В этом случае предлагаем использовать код для входа
      console.log(
        '⚠️ Пользователь не имеет пароля, но пытается войти с паролем',
      );
      throw new UnauthorizedException(
        'Пользователь не имеет пароля. Используйте код для входа или установите пароль в настройках.',
      );
    }
  ...
  async completeOnboarding(
    ...
    if (dto.password) {
      ...
      console.log(`🔐 Пароль установлен для пользователя ${user.email}`);
    }

    await this.usersService.update(userId, updateData);
    
    return { message: 'Онбординг завершен успешно' };
  }
   ...
   */
  async generateRefreshToken(user: User, userAgent?: string, ipAddress?: string): Promise<string> {
  ...
  async getCurrentUser(userId: string): Promise<any> {
    ...
    try {
      ...
      // Извлекаем глобальную роль для фронтенда
      const globalRole = roles.find((r: any) => r?.isGlobal && !r?.organizationId && !r?.teamId);
      const roleName = globalRole?.name || 'user';
      ...
      const result = {
        ...
        roles: roles,
        role: roleName, // Добавляем роль в формате, ожидаемом фронтендом
