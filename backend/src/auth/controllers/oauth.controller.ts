...
export class OAuthController {
  ...
  async authorize(
    ...
    // ✅ КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Если это OAuth flow (есть OAuth cookies), 
    // игнорируем Authorization header, чтобы не использовать сохраненный токен от предыдущего входа
    // Это решает проблему, когда пользователь входит во вселенную через email, 
    // а затем пытается войти через GitHub/Telegram - система должна использовать пользователя из OAuth, 
    // а не сохраненный токен от email входа
    const isOAuthFlow = !!req.cookies?.oauth_flow_active || !!req.cookies?.oauth_client_id || !!req.cookies?.oauth_redirect_uri;
    console.log(`🔍 [OAuth] Is OAuth flow active: ${isOAuthFlow}`);
    ...
    // ✅ КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Если это OAuth flow, НЕ используем Authorization header
    // Это гарантирует, что при входе через GitHub/Telegram система не будет использовать
    // сохраненный токен от предыдущего входа через email
    // Если temp_token не помог и это НЕ OAuth flow, проверяем Authorization header
    if (!user && !isOAuthFlow) {
    ...
    } else if (!user && isOAuthFlow) {
      console.log(`🔍 [OAuth] OAuth flow detected - ignoring Authorization header to prevent using saved token from previous email login`);
    }
