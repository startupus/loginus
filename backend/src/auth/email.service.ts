...
export class EmailService implements OnModuleInit {
  ...
  private async createTransporter() {
    ...
    const isSmtpConfigured = smtpPassword && 
    ...
    // В dev-режиме, если SMTP не настроен, используем Ethereal Email (тестовый SMTP)
    if (isDev && !isSmtpConfigured) {
      console.log('📧 SMTP не настроен. Используем Ethereal Email (тестовый SMTP) для разработки...');
      try {
        ...
        console.log('✅ Ethereal Email аккаунт создан:');
        console.log(`   Email: ${this.etherealAccount.user}`);
        console.log(`   Пароль: ${this.etherealAccount.pass}`);
        console.log(`   SMTP: ${this.etherealAccount.smtp.host}:${this.etherealAccount.smtp.port}`);
        console.log(`   Web UI: https://ethereal.email`);
        console.log('   📧 Письма будут отправляться на тестовый аккаунт. Ссылки на письма будут в логах.');
        
        // Проверяем соединение
        await this.transporter.verify();
        console.log('✅ Ethereal Email SMTP Server is ready to take our messages');
        return;
      } catch (error) {
        console.error('❌ Ошибка создания Ethereal Email аккаунта:', error);
        console.error('   Продолжаем с обычным SMTP (письма не будут отправляться)');
      }
    }
    ...
    if (!smtpPassword || smtpPassword === 'your-app-password') {
      if (isDev) {
        console.warn('⚠️  SMTP_PASSWORD не настроен. В dev-режиме используется Ethereal Email.');
      } else {
  ...
  async sendVerificationCode(email: string, code: string, method?: string): Promise<void> {
    ...
    try {
      ...
      // Если используется Ethereal Email, выводим ссылку на письмо
      ...
      // В dev-режиме не пробрасываем ошибку, чтобы процесс продолжался
      if (isDev) {
        console.warn('⚠️  Dev-режим: продолжаем без отправки email. Код доступен в логах выше.');
        return; // Не пробрасываем ошибку в dev-режиме
      }
      ...
      throw error; // В production пробрасываем ошибку
    }
  }
  ...
  async sendEmail(options: { to: string; subject: string; html: string }): Promise<void> {
    ...
    // Извлекаем код из HTML для логирования
    const codeMatch = options.html.match(/<h1[^>]*>(\d{6})<\/h1>/);
    ...
    try {
      ...
      console.log(`   ╔═══════════════════════════════════════════════════════════╗`);
      console.log(`   ║  EMAIL НЕ ОТПРАВЛЕН (SMTP ошибка), но код доступен:        ║`);
      console.log(`   ║  To: ${options.to.padEnd(47)} ║`);
      console.log(`   ║  Subject: ${options.subject.padEnd(43)} ║`);
      if (code) {
        console.log(`   ║  Code: ${code.padEnd(49)} ║`);
      }
      console.log(`   ╚═══════════════════════════════════════════════════════════╝`);
      ...
      // В production пробрасываем ошибку
      throw error;
    }
  }
  ...
  /**
   * Отправка уведомления о настройке способа восстановления
   */
  async sendRecoveryMethodSetupEmail(email: string, method: 'email' | 'phone'): Promise<void> {
    const methodLabel = method === 'email' ? 'Email' : 'Телефон';
    const subject = `✅ Способ восстановления настроен - Loginus`;
    const html = this.generateRecoveryMethodSetupTemplate(methodLabel, email);
    ...
    try {
      ...
      console.log(`📧 Уведомление о настройке способа восстановления отправлено на ${email}`);
    } catch (error) {
      console.error('❌ Ошибка отправки уведомления о настройке способа восстановления:', error);
    }
  }

  /**
   * Генерация HTML шаблона для уведомления о настройке способа восстановления
   */
  private generateRecoveryMethodSetupTemplate(methodLabel: string, contact: string): string {
    return `
      <!DOCTYPE html>
      <html>
      <head>
        <meta charset="utf-8">
        <title>Способ восстановления настроен - Loginus</title>
        <style>
      ...
      <body>
        <div class="container">
          <div class="header">
            <h1 style="color: #667eea;">✅ Способ восстановления настроен</h1>
          </div>
          <div class="content">
            <p>Здравствуйте!</p>
            <div class="success-box">
              <p style="margin: 0; font-weight: bold; color: #065F46;">Восстановление через ${methodLabel} успешно настроено!</p>
            </div>
            <p>Ваш ${methodLabel.toLowerCase()} <strong>${contact}</strong> теперь используется как способ восстановления доступа к вашему аккаунту Loginus.</p>
            <div class="info-box">
              <p style="margin: 0; color: #0C4A6E;"><strong>Что это значит?</strong></p>
              <p style="margin: 10px 0 0 0; color: #0C4A6E;">Если вы забудете пароль, мы отправим вам ссылку для восстановления на этот ${methodLabel.toLowerCase()}.</p>
            </div>
            <p>Если вы не настраивали этот способ восстановления, пожалуйста, <a href="mailto:support@loginus.ru" style="color: #667eea;">свяжитесь с поддержкой</a>.</p>
          </div>
          <div class="footer">
