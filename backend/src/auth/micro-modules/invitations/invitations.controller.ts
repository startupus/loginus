import { Controller, Post, Get, Delete, Body, Param, UseGuards, Query, BadRequestException } from '@nestjs/common';
...
export class InvitationsController {
  ...
  @Get('details')
  @Public()
  @ApiOperation({ summary: 'Получить детали приглашения по токену (публичный)' })
  @ApiResponse({ status: 200, description: 'Детали приглашения' })
  ...
  async getInvitationDetails(
    ...
    console.log('🔍 [InvitationsController] getInvitationDetails called with token:', token ? token.substring(0, 20) + '...' : 'null');
    if (!token) {
      console.error('❌ [InvitationsController] Token is missing');
      throw new BadRequestException('Токен приглашения обязателен');
    }
    try {
      const details = await this.invitationsService.getInvitationDetails(token);
      console.log('✅ [InvitationsController] Invitation details retrieved:', { 
      ...
      console.error('❌ [InvitationsController] Error getting invitation details:', error);
      throw error;
    }
  }
  ...
  async acceptInvitation(
    ...
    console.log(`[InvitationsController] acceptInvitation called, currentUser object:`, currentUser ? JSON.stringify(Object.keys(currentUser)) : 'null');
    console.log(`[InvitationsController] acceptInvitation called, currentUser.userId:`, currentUser?.userId);
    console.log(`[InvitationsController] acceptInvitation called, currentUser.id:`, currentUser?.id);
    console.log(`[InvitationsController] acceptInvitation called, currentUser.sub:`, currentUser?.sub);
    const userId = currentUser?.userId || currentUser?.id || currentUser?.sub;
    console.log(`[InvitationsController] acceptInvitation called, extracted userId:`, userId || 'null');
    return this.invitationsService.acceptInvitation(dto, userId);
  }
  ...
  async acceptInvitationFromNotification(
    ...
    console.log(`🎯 CONTROLLER: acceptInvitationFromNotification called with userId=${user.userId}, invitationId=${invitationId}`);
