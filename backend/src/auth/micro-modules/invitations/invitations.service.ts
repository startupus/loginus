...
import { UserFamilyGroup, FamilyMemberRole } from '../../../family/entities/user-family-group.entity';
...
export class InvitationsService {
  ...
  async createInvitation(
    ...
    if (existingUser) {
      ...
      // Создаем приглашение
      // ВАЖНО: Для семейных групп сохраняем roleName как строку в поле role
      // Это нужно для правильного определения роли при принятии приглашения
      let roleValue: string | undefined = roleId || undefined;
      if (dto.type === InvitationType.FAMILY_GROUP && dto.roleName) {
        // Для семейных групп сохраняем roleName напрямую как строку
        roleValue = dto.roleName;
        console.log(`🔍 Saving roleName '${dto.roleName}' as role for family group invitation`);
      }
    ...
    if (existingInvitation) {
      // Вместо ошибки возвращаем уже существующее приглашение
      // Это делает операцию идемпотентной: повторная отправка приглашения
      // для того же пользователя просто возвращает ту же ссылку
      const frontendUrl = this.configService.get<string>('FRONTEND_URL');
      const token = existingInvitation.token;
      ...
      return {
        ...
        invitedById: existingInvitation.invitedById,
        acceptedById: existingInvitation.acceptedById,
        token: existingInvitation.token,
        expiresAt: existingInvitation.expiresAt,
        acceptedAt: existingInvitation.acceptedAt,
        createdAt: existingInvitation.createdAt,
        updatedAt: existingInvitation.updatedAt,
        invitationLink,
      };
    }
   ...
   */
  async acceptInvitation(dto: AcceptInvitationDto, currentUserId?: string): Promise<{ 
    success: boolean; 
    userId?: string; 
    redirectTo?: string;
    message?: string;
  }> {
    console.log(`🔍 Starting invitation acceptance for token: ${dto.token}, currentUserId: ${currentUserId || 'null'}`);
    ...
    // Логируем familyGroupId для отладки
    if (invitation) {
      console.log(`🔍 Invitation found: type=${invitation.type}, familyGroupId: ${invitation.familyGroupId || 'null'}, email: ${invitation.email || 'null'}`);
    }
    
    console.log(`🔍 Invitation found:`, invitation ? { id: invitation.id, email: invitation.email, organizationId: invitation.organizationId, teamId: invitation.teamId, type: invitation.type } : 'null');
    ...
    // Для семейных групп: кто перешел по ссылке, тот и присоединяется
    // Для остальных типов: используем email из приглашения
    let user;
    if (invitation.type === InvitationType.FAMILY_GROUP) {
      // Для семейных групп ОБЯЗАТЕЛЬНО требуется авторизованный пользователь
      if (!currentUserId) {
        console.error(`❌ FAMILY_GROUP invitation requires authentication, but currentUserId is null`);
        throw new BadRequestException('Для принятия приглашения в семейную группу необходимо быть авторизованным. Пожалуйста, войдите в систему и попробуйте снова.');
      }
      user = await this.usersService.findById(currentUserId);
      if (!user) {
        throw new NotFoundException('Пользователь не найден');
      }
      console.log(`✅ Используем авторизованного пользователя ${user.email} (ID: ${user.id}) для семейной группы`);
    } else if (currentUserId) {
      // Для других типов приглашений, если пользователь авторизован, используем его
      user = await this.usersService.findById(currentUserId);
      if (!user) {
        throw new NotFoundException('Пользователь не найден');
      }
      console.log(`✅ Используем авторизованного пользователя ${user.email} (ID: ${user.id})`);
    } else {
      // Для других типов приглашений, если пользователь не авторизован, используем email
      if (!invitation.email) {
        throw new BadRequestException('Email обязателен для данного типа приглашения');
      }
      user = await this.usersService.findByEmail(invitation.email as string);
    ...
    // Добавляем пользователя в семейную группу, если указана
    console.log(`🔍 Checking invitation.familyGroupId: ${invitation.familyGroupId || 'null'}`);
    if (invitation.familyGroupId) {
      console.log(`🔍 Adding user ${user.email} to family group ${invitation.familyGroupId}`);
      
      // Проверяем, не является ли пользователь уже членом семейной группы
      const existingFamilyMembership = await this.userFamilyGroupRepo.findOne({
        where: {
          userId: user.id,
      ...
      if (!existingFamilyMembership) {
        // Определяем роль в семейной группе на основе roleName из приглашения
        // ВАЖНО: Используем roleName из приглашения, который был установлен при создании
        let familyRole = FamilyMemberRole.MEMBER;
        
        // Проверяем roleName из приглашения (если он есть в DTO, он должен быть сохранен)
        // Для семейных групп roleName хранится в поле role как строка 'child' или 'member'
        if (invitation.role) {
          const roleName = typeof invitation.role === 'string' ? invitation.role : (invitation.role as any)?.name;
          console.log(`🔍 Invitation role: ${roleName}`);
          if (roleName === 'child') {
            familyRole = FamilyMemberRole.CHILD;
            console.log(`✅ Setting family role to CHILD for user ${user.email}`);
        ...
        const familyMembership = this.userFamilyGroupRepo.create({
          userId: user.id,
          familyGroupId: invitation.familyGroupId,
          role: familyRole,
          invitedBy: invitation.invitedById,
        ...
        await this.userFamilyGroupRepo.save(familyMembership);
        console.log(`✅ User ${user.email} added to family group ${invitation.familyGroupId} with role ${familyRole}`);
      } else {
        console.log(`ℹ️ User ${user.email} is already a member of family group ${invitation.familyGroupId}`);
      }
    }

    // Назначаем роль из приглашения (если указана) или базовую роль
    // ВАЖНО: Для семейных групп не назначаем глобальные роли - роль хранится в user_family_groups
    if (invitation.type !== InvitationType.FAMILY_GROUP && invitation.role) {
    ...
    } else if (invitation.type !== InvitationType.FAMILY_GROUP) {
      // Если в приглашении не указана роль, назначаем базовую роль "viewer" (только для организаций/команд)
      ...
      console.log(`ℹ️ Для семейных групп роль хранится в user_family_groups, пропускаем назначение глобальной роли`);
    }
    ...
    console.log(`✅ Пользователь ${user.email} (ID: ${user.id}) принял приглашение от ${invitation.invitedBy.email}`);
    console.log(`✅ Приглашение обновлено: status=ACCEPTED, organizationId=${invitation.organizationId}, teamId=${invitation.teamId}`);
    
    // Проверяем, что пользователь действительно добавлен
    if (invitation.organizationId) {
      const checkMembership = await this.orgMembershipRepo.findOne({
        where: { userId: user.id, organizationId: invitation.organizationId }
      });
      console.log(`🔍 Проверка членства в организации: ${checkMembership ? '✅ Найдено' : '❌ Не найдено'}`);
    }
    if (invitation.teamId) {
      const checkMembership = await this.teamMembershipRepo.findOne({
        where: { userId: user.id, teamId: invitation.teamId }
      });
      console.log(`🔍 Проверка членства в команде: ${checkMembership ? '✅ Найдено' : '❌ Не найдено'}`);
    }
  ...
  async getUserInvitations(userId: string): Promise<InvitationResponseDto[]> {
    ...
    } as InvitationResponseDto & { targetName?: string; roleName?: string; inviterName?: string; inviterEmail?: string | null }));
  }
  ...
  async getSentInvitations(userId: string): Promise<InvitationResponseDto[]> {
    ...
    return invitations.map((invitation: any) => ({
      ...
      organizationId: invitation.organizationId,
      teamId: invitation.teamId,
      familyGroupId: invitation.familyGroupId || invitation.family_group_id || null, // Добавляем familyGroupId
      status: invitation.status,
      role: invitation.role,
      invitedById: invitation.invitedById,
      acceptedById: invitation.acceptedById,
  ...
  /**
   * Получить детали приглашения по токену (публичный метод)
   */
  async getInvitationDetails(token: string): Promise<{
    ...
    const invitation = await this.invitationsRepo.findOne({
      where: { token },
      relations: ['invitedBy', 'organization', 'team'],
    });
    ...
    const inviterName: string = invitation.invitedBy
      ? `${invitation.invitedBy.firstName || ''} ${invitation.invitedBy.lastName || ''}`.trim() || invitation.invitedBy.email || 'Неизвестный пользователь'
      : 'Неизвестный пользователь';

    // Для семейных групп email может быть null, используем дефолтное имя
    const targetName: string = (invitation.email ?? 'Пользователь') as string;

    // Получаем название роли
    let roleName = 'Участник';
    if (invitation.role) {
      // Если есть roleId, пытаемся найти роль
      if (invitation.type === InvitationType.TEAM && invitation.teamId) {
        const teamRole = await this.teamRoleRepo.findOne({
          where: { id: invitation.role, teamId: invitation.teamId },
        });
        if (teamRole) {
          roleName = teamRole.name;
        }
      } else if (invitation.type === InvitationType.ORGANIZATION && invitation.organizationId) {
        const orgRole = await this.orgRoleRepo.findOne({
          where: { id: invitation.role, organizationId: invitation.organizationId },
        });
        if (orgRole) {
  ...
  async handleInvitationLink(token: string): Promise<{
    ...
    // Для семейных групп email может быть null, поэтому проверяем только если email есть
    let existingUser: User | null = null;
    if (invitation.email) {
      existingUser = await this.usersService.findByEmail(invitation.email);
    }
  ...
  async createInternalInvitation(
    ...
    // Если передан roleName, находим roleId для конкретной команды/организации
    // Если роль не найдена, просто не передаем roleId - роль будет назначена по умолчанию при принятии приглашения
    let roleId = dto.roleId;
    if (dto.roleName && !dto.roleId) {
      if (dto.type === InvitationType.TEAM && dto.teamId) {
        const teamRole = await this.teamRoleRepo.findOne({
        ...
        if (teamRole) {
          ...
          // Роль не найдена - не критично, будет назначена по умолчанию при принятии
          console.log(`⚠️ Роль '${dto.roleName}' не найдена в команде ${dto.teamId}, приглашение будет создано без роли`);
          roleId = undefined;
        }
      } else if (dto.type === InvitationType.ORGANIZATION && dto.organizationId) {
        const orgRole = await this.orgRoleRepo.findOne({
        ...
        if (orgRole) {
          ...
          // Роль не найдена - не критично, будет назначена по умолчанию при принятии
          console.log(`⚠️ Роль '${dto.roleName}' не найдена в организации ${dto.organizationId}, приглашение будет создано без роли`);
          roleId = undefined;
        }
      }
    }

    // Для семейных групп не требуем email - кто перешел по ссылке, тот и присоединяется
    if (dto.type === InvitationType.FAMILY_GROUP) {
      // Генерируем токен и создаем приглашение без привязки к email
      const token = crypto.randomBytes(32).toString('hex');
      const expiresInDays = dto.expiresInDays || 30; // Для семейных групп делаем срок действия больше
      const expiresAt = new Date();
      expiresAt.setDate(expiresAt.getDate() + expiresInDays);

      // Сохраняем roleName как строку для семейных групп
      let roleValue: string | undefined = dto.roleName || undefined;
      
      const invitation = this.invitationsRepo.create({
        email: null, // Не привязываем к email
        type: dto.type,
        familyGroupId: dto.familyGroupId,
        role: roleValue,
        invitedById,
      ...
      const result: InvitationResponseDto = {
        id: invitation.id,
        email: null,
        type: invitation.type,
        familyGroupId: invitation.familyGroupId,
    ...
    // Для остальных типов приглашений требуем email
    if (!dto.email) {
      throw new BadRequestException('Email обязателен для данного типа приглашения');
    }
    ...
    const invitation = this.invitationsRepo.create({
      ...
      organizationId: dto.organizationId,
      teamId: dto.teamId,
      familyGroupId: dto.familyGroupId, // Добавляем familyGroupId для семейных групп
      role: roleId || undefined,
      invitedById,
      token,
      expiresAt,
    ...
    const result: InvitationResponseDto = {
      ...
      organizationId: invitation.organizationId,
      teamId: invitation.teamId,
      familyGroupId: invitation.familyGroupId, // Добавляем familyGroupId в ответ
      status: invitation.status,
      role: invitation.role,
      invitedById: invitation.invitedById,
      acceptedById: invitation.acceptedById,
