...
export class User {
  ...
  @Column({ type: 'varchar', length: 255, nullable: true, name: 'githubemail' })
  githubEmail: string | null; // Email от GitHub (отдельно от основного email)

  @Column({ type: 'varchar', length: 20, nullable: true, name: 'telegramphone' })
  ...
  // Тип аутентификации для EMAIL (пароль или код)
  @Column({ type: 'varchar', length: 20, nullable: true, default: 'password' })
  emailAuthType: 'password' | 'code' | null;

  // Флаг, указывающий, включен ли email-code как дополнительный фактор (когда выбраны оба метода)
