...
- Переводы в модульной структуре через API v2 (`services/i18n/v2/`)
- Структура ключей: `section.subsection.key`
- **Оптимизированная загрузка модулей**:
  - Критичные модули (common, profile, dashboard) загружаются при инициализации
  - Остальные модули загружаются по требованию (ленивая загрузка)
  - Используется IndexedDB кэш для быстрой загрузки
  - При переключении языка загружается только 3 критичных модуля
- **Производительность**: 
  - Время переключения языка: < 500ms
  - При первой загрузке: 3-5 запросов к API переводов
...
При работе с переводами в компонентах дизайн-системы:

1. **Используйте useMemo с правильными зависимостями**:
   ```tsx
   // ✅ Правильно - только необходимые зависимости
   const items = useMemo(() => [
     { label: t('sidebar.profile'), path: buildPathWithLang('/dashboard', currentLang) }
   ], [t, currentLang, location.pathname]);
   
   // ❌ Неправильно - лишние зависимости
   const items = useMemo(() => [...], [t, currentLang, i18n.language]);
   ```

2. **Предзагрузка модулей**: Используйте `preloadModule()` для не критичных модулей
3. **Кэширование**: Модули автоматически кэшируются в IndexedDB
4. **Ленивая загрузка**: Не критичные модули загружаются по требованию

### Компоненты с оптимизацией

- `PageTemplate` - оптимизированы зависимости useMemo для Sidebar
- `LanguageSwitcher` - единый компонент для переключения языка
- `Sidebar` - использует переводы из модуля `common`
...
5. Оптимизируйте useMemo зависимости (не включайте `i18n.language` если не нужно)
