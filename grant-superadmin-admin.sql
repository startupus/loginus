-- Выдача роли super_admin пользователю admin@admin.ru
DO $$
DECLARE
    v_user_id UUID;
    v_role_id UUID;
...
BEGIN
    ...
    SELECT id INTO v_user_id FROM users WHERE email = 'admin@admin.ru';
    
    IF v_user_id IS NULL THEN
        RAISE EXCEPTION 'User with email admin@admin.ru not found';
    END IF;
    ...
    IF v_assignment_id IS NULL THEN
        ...
        RAISE NOTICE 'Granted super_admin role to user admin@admin.ru';
    ELSE
        RAISE NOTICE 'User already has super_admin role assigned';
    END IF;
    
    RAISE NOTICE 'Success! User admin@admin.ru now has super_admin role';
END $$;
