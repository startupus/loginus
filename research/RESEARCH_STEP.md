...
## Research – Оптимизация админских таблиц и i18n-потока (2025-11-24)

### Context
- Пользователь сообщает, что страницы (особенно админские) долго грузятся при большом ответе API.
- Дополнительно требуется провести повторный аудит i18n на страницах `/en/*`, убедиться в корректной загрузке модулей, URL и форматирования.
- Исходные данные: React + TanStack Query, таблицы со множеством фильтров, переводные строки из API v2 (`/api/v2/translations`), переключение языка через `LanguageSwitcher`.

### External references
- TanStack Query best practices по работе с `useInfiniteQuery`, ленивой подгрузкой страниц и ограничением числа хранящихся страниц: `/tanstack/query` → infinite queries руководство (Context7, 2025-11-24).
- Виртуализация таблиц с помощью `react-window` (FixedSizeList / VariableSizeGrid): `/websites/react-window_vercel_app` (Context7, 2025-11-24).
- Кеширование и цепочки backend'ов в i18next (localStorageBackend / HttpBackend): `/websites/i18next` (Context7, 2025-11-24).

### Findings
1. **Объём данных из API**: текущая реализация `UsersManagementPage` загружает полные списки компаний и пользователей и выполняет поиск/фильтры в памяти. При десятках тысяч записей это ведёт к долгим вычислениям и повторным `find` внутри `render`.
2. **Отсутствие виртуализации**: таблица пользователей полностью рендерит все строки страницы. При больших лимитах (или выключенной пагинации) Virtual DOM перегружается.
3. **Повторные вычисления**: `companies.find` вызывается для каждой строки, что приводит к `O(n * m)` при росте данных.
4. **i18n**: на ряде страниц встречаются fallback-строки и форматирование не завязано на текущую локаль. Нужно удостовериться в загрузке модулей и корректном использовании `t()`/`formatDate`.
5. **Кеширование переводов**: i18next уже использует IndexedDB, но стоит подтвердить настройку TTL и очистку при смене языка, чтобы избежать мигания.

### Best practices summary
- Разделять загрузку данных на страницы (`useInfiniteQuery`, `getNextPageParam`, `maxPages`) либо серверную пагинацию с лимитом в API. Хранить не более 3–5 страниц в памяти, очищать остальное (`queryClient.setQueryData`).
- Виртуализировать вывод таблиц (`FixedSizeList` / `VariableSizeList`) или хотя бы использовать `React.memo`/`useMemo` для тяжёлых строк.
- Предподготавливать справочники (например, `companyMap: Record<string, Company>`) вне рендера через `useMemo`.
- Для i18n: проверять, что язык берётся из URL, `document.documentElement.lang` обновляется, `t()` вызывается внутри рендера, числа/валюты форматируются через `Intl` c `locale`.
- Кэшировать переводы в локальном storage + HTTP backend и сбрасывать кеш при `languageChanged`, чтобы исключить обращение к статике.

### Plan & opportunities
1. **Оптимизировать `UsersManagementPage`**: 
   - Перейти с ручной пагинации на серверную (уже есть параметры `page/limit`, нужно согласовать формат ответа) либо применить `useInfiniteQuery`.
   - Добавить мемоизированные словари для `companies`, вынести фильтрацию на сервер (либо предварительный `useMemo`).
   - Рассмотреть внедрение `react-window` для таблицы, особенно для режимов >20 строк на страницу.
2. **Аудит i18n**: пройти по `/en/dashboard`, `/en/data`, `/en/support`, `/en/admin/backup`, проверить загрузки модулей, форматирование и URL (по чек-листу пользователя).
3. **Инфраструктура переводов**: подтвердить работу API v2, проверить `LanguageRoute` и обновить тесты Playwright (`i18n-language-switch.spec.ts` + новые кейсы для админки).
4. **Документация и тесты**: задокументировать оптимизации (README разделов, `TEST_RESULTS.md`), добавить e2e-тест на сохранение языка/форматирования.

### Risks
- Требуется аккуратно изменить контракт ответа `/admin/users`, чтобы фронт работал с `data.users` и `meta`. Понадобится двусторонняя синхронизация с mock.
- Виртуализация влияет на модальные окна и хоткеи (нужно протестировать управление фокусом).
- Ограничение кеша переводов может привести к лишним запросам; важно сбалансировать TTL.
